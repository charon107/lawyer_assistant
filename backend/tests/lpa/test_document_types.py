...
ALL_TYPES = ["lpa", "contract", "nda", "employment"]

REQUIRED_CONFIG_KEYS = [
    "name", "report_title", "chapter_keywords", "entity_patterns",
    "fact_tool_schema", "risk_rules", "rule_keyword_map",
    "simple_rule_ids", "complex_rule_ids", "prompt_templates",
]

REQUIRED_PROMPT_KEYS = [
    "chapter_split", "fact_labeling", "simple_review", "complex_review", "cross_check",
]
...
class TestRiskRulesStructure:
    ...
    def test_rules_have_required_fields(self, doc_type: str):
        ...
        for rule_id, rule in rules.items():
            ...
            assert "suggestion_template" in rule, f"'{doc_type}' rule {rule_id} missing 'suggestion_template'"
            assert rule["id"] == rule_id, f"'{doc_type}' rule id mismatch: {rule['id']} != {rule_id}"
    ...
    def test_rule_levels_are_valid(self, doc_type: str):
        ...
        rules = DOCUMENT_TYPE_CONFIGS[doc_type]["risk_rules"]
        for rule_id, rule in rules.items():
            assert rule["level"] in valid_levels, f"'{doc_type}' rule {rule_id} invalid level: {rule['level']}"
...
class TestRuleKeywordMap:
    ...
    def test_keyword_map_keys_in_rules(self, doc_type: str):
        ...
        for rule_id in kw_map:
            assert rule_id in rules, f"'{doc_type}' keyword map references non-existent rule '{rule_id}'"
