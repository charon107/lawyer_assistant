"""IP-legal module WebSocket endpoint.

`/ws/ip` — runs one ip-legal Agent skill per WS message. Mirrors the
privacy-legal WS: no cross-run history, fresh Agent per message, same
streaming event names. The skill is chosen explicitly by the client
(`action`) — no LLM intent routing.

Actions:
    Review-producing (pre-create an ip_reviews row):
        clearance, fto, invention, infringement, ip_clause, oss
    Enforcement (needs enforcement_id):
...
Client → Server:
    {
        ...
        "subject": "<mark / product / counterparty>",  # optional, for review actions
        "enforcement_id": "...",   # required for cease_desist / takedown
    }
...
from app.repositories import ip_enforcement_repo, ip_profile_repo, ip_review_repo
...
_REVIEW_PRECREATE_ACTIONS: frozenset[str] = frozenset(
    {"clearance", "fto", "invention", "infringement", "ip_clause", "oss"}
)
_ENFORCEMENT_ACTIONS: frozenset[str] = frozenset({"cease_desist", "takedown"})

_VALID_ACTIONS: frozenset[str] = _REVIEW_PRECREATE_ACTIONS | _ENFORCEMENT_ACTIONS
...
async def _run_skill(
    *, websocket: WebSocket, user: User, data: dict[str, Any], action: str
    ...
    skill: IpSkillName = action  # type: ignore[assignment]
    ...
    # Resolve + ownership-check the enforcement record before running.
    if action in _ENFORCEMENT_ACTIONS:
        enforcement_id = data.get("enforcement_id")
        if not enforcement_id:
            await manager.send_event(websocket, "error", {"message": "enforcement_id is required"})
            return
        with contextmanager(get_db_session)() as db:
            row = ip_enforcement_repo.get_by_id(db, enforcement_id)
            if row is None or row.user_id != str(user.id):
                await manager.send_event(
                    websocket,
                    "error",
                    {"message": "Enforcement matter not found or not owned by you"},
                )
                return

    with contextmanager(get_db_session)() as db:
        profile = ip_profile_repo.get_by_user_id(db, str(user.id))
        profile_md = profile.profile_content if profile else None

        review_id: str | None = None
        if action in _REVIEW_PRECREATE_ACTIONS:
            row = ip_review_repo.create(
                db,
                user_id=str(user.id),
                review_type=review_type or "clearance",
        ...
        deps = IpDeps(
            ...
            output_dir=str(Path("outputs") / "ip" / str(user.id)),
        )
