"""Cold-start interview state machine for the employment-legal module.

Six steps (see schemas/employment/cold_start.py):

    0 = role + practice scenario
    1 = jurisdiction scope (省/直辖市 multi-select)
    2 = review triggers (hiring / termination)
    3 = high-risk flags + severance policy + escalation matrix
    4 = seed files (informational)
    5 = generate profile

State lives in `module_configs.setup_data` (JSON). On the final step the
accumulated answers are compiled into an `employment_profiles` row.
...
from app.repositories import employment_profile_repo, module_config_repo
from app.schemas.employment.cold_start import (
    ColdStartRequest,
    ColdStartResponse,
    ColdStartStep,
)

MODULE_NAME = "employment-legal"
TOTAL_STEPS = 6
...
def _compile_profile_kwargs(setup_data: dict[str, Any]) -> dict[str, Any]:
    """Project accumulated step answers into employment_profile_repo kwargs."""
    ...
    kwargs: dict[str, Any] = {
        ...
        "jurisdictions": juris.get("jurisdictions"),
        "default_jurisdiction": juris.get("default_jurisdiction"),
        "office_model": juris.get("office_model") or "in_office",
        "hiring_trigger": triggers.get("hiring_trigger"),
        "termination_trigger": triggers.get("termination_trigger"),
        "standard_severance": risk.get("standard_severance"),
        "high_risk_flags": risk.get("high_risk_flags"),
        "escalation_matrix": risk.get("escalation_matrix"),
...
class EmploymentColdStartService:
    ...
    def _materialize_profile(self, user_id: str, setup_data: dict[str, Any]) -> None:
        ...
        if existing is None:
            employment_profile_repo.create(self.db, user_id=user_id, **kwargs)
        else:
            employment_profile_repo.update(self.db, profile=existing, **kwargs)
