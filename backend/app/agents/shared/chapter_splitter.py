...
# Generic contract keywords (Chinese + English) - not domain-specific
...
class ChapterSplitter:
    ...
    def _split_by_keyword(self, text: str) -> list[dict[str, Any]]:
        ...
        for i, line_idx in enumerate(boundary_indices):
            char_start = sum(len(ln) + 1 for ln in lines[:line_idx])
            next_idx = boundary_indices[i + 1] if i + 1 < len(boundary_indices) else len(lines)
            next_char = sum(len(ln) + 1 for ln in lines[:next_idx])
            chapters.append(
                {
                    "index": i + 1,
    ...
    def _split_by_llm(self, text: str) -> list[dict[str, Any]]:
        ...
        assert self._llm is not None, "_split_by_llm called without llm_client"
        ...
        try:
            ...
            data = parse_json(resp) if isinstance(resp, str) else resp
