...
class LLMClient:
    ...
    def __init__(
        ...
        # Fix: create client once, reuse connection pool
        from openai import OpenAI

        self._client = OpenAI(api_key=api_key, base_url=base_url, timeout=_API_TIMEOUT)

    def register_tool(self, name: str, description: str, input_schema: dict, handler: Callable):
        """Register a tool at instance level. Deduplicates by name."""
        # Fix: remove old registration with same name to prevent accumulation
    ...
    def agent_loop(
        ...
        for t in openai_tools:
            logger.info(
                "  tool: %s — %s", t["function"]["name"], t["function"]["description"][:200]
            )
            logger.debug(
                "  parameters: %s",
                json.dumps(t["function"].get("parameters", {}), ensure_ascii=False)[:500],
            )
        ...
        for turn in range(max_turns):
            ...
            if choice.finish_reason == "tool_calls":
                ...
                for tc in choice.message.tool_calls:
                    ...
                    logger.info(
                        "EXECUTING tool: %s (handler=%s)", tc.function.name, handler is not None
                    )
                    if handler is None:
                        ...
                        try:
                            ...
                            if not args:
                                ...
                                result = json.dumps(
                                    {"error": "参数为空。请根据合同内容填写实际值后重新调用工具。"},
                                    ensure_ascii=False,
                                )
                            else:
                                result = handler(**args)
            ...
            logger.warning(
                "agent_loop unexpected finish_reason=%s, returning content", choice.finish_reason
            )
            return choice.message.content or ""
    ...
    def chat(
        ...
        logger.info(
            "chat() response: content_len=%d, finish_reason=%s",
            len(content),
        ...
        if content:
            ...
            logger.warning(
                "chat() returned EMPTY content, finish_reason=%s", resp.choices[0].finish_reason
            )
        return content
