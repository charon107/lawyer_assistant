...
- **监管合规模块（Regulatory-Legal，第 7 个法律模块）**: 面向中国行政法的法规动态监测 + 政策差距分析 + 合规差距追踪，全栈落地（后端 Phase 1-4 + 前端 9 页）。
  - **法规动态监测**: 「立即检查」一键拉取已配置的 RSS/Atom/JSON 法规源，按重要度三层级（🔴始终重要 / 🟡值得审阅 / 🟢仅供参考）分层呈现；支持手动粘贴录入；每条带来源溯源标签与法规状态验证标记。
  - **政策差异分析（policy-diff）**: 对照法规变化与政策库逐要求分析差距，法规未现行有效时输出加未验证横幅，差距按法条引用去重后交接至追踪器。
  - **差距追踪器**: 六分区状态报告（逾期 / 30 天内 / 开放 / 👀观察事项 / 进行中 / 最近关闭），观察事项（前瞻性）与合规差距分离呈现；支持关闭与风险接受（保留不删）。
  - **意见征集追踪器**: 征求意见稿决策记录（拟提交 / 不提交 / 已提交 / 放弃）与截止跟踪。
  - **政策重述草案（policy-redraft）**: 产出带标记修订的政策重述备忘录，写入新文件不覆盖源、不在追踪器关闭差距。
  - **冷启动向导**: 六步学习监测清单 / 重要度阈值 / 政策库 / 动态源配置；设置页支持 customize 编辑。
  - **法规动态监测定时代理（reg-change-monitor）**: 每周一确定性抓取（不调 LLM、不外发），区分「源返回空」与「抓取失败」，按重要度预分桶，自动登记征求意见稿，发布站内简报。
