...
export function useDocumentReview() {
  ...
  const connectWS = useCallback((reviewId: string, apiBase: string) => {
    const url = `${getWsUrl()}${apiBase}/review/${reviewId}/ws`;
      ...
      ws.onmessage = (event) => {
        ...
        setState((prev) => {
          switch (data.event) {
            ...
            case "chapter_result": {
              // Deduplicate: skip if this chapter already exists
              const exists = prev.chapterReviews.some((r: any) => r.chapter === data.chapter);
  ...
  const startReview = useCallback(
    async (file: File, apiBase: string) => {
      setState({
        reviewId: null,
        status: "uploading",
        progress: 0,
  ...
  const confirmChapters = useCallback(
    async (chapters: any[], apiBase: string) => {
      if (!state.reviewId) return;

      try {
        const res = await fetch(`${apiBase}/review/${state.reviewId}/chapters`, {
          method: "PUT",
  ...
  const fetchReport = useCallback(
    async (apiBase: string) => {
      ...
      try {
        const res = await fetch(`${apiBase}/review/${state.reviewId}/report`, {
          headers: getAuthHeaders(),
        });
  ...
  const fetchFullResult = useCallback(
    async (apiBase: string, id?: string) => {
      ...
      try {
        const res = await fetch(`${apiBase}/review/${rid}/full`, {
          headers: getAuthHeaders(),
        });
        if (!res.ok) return null;
        const data = await res.json();
  ...
  const sendChatMessage = useCallback(
    async (question: string, history: any[], apiBase: string): Promise<string> => {
      if (!state.reviewId) return "审查会话不存在";
      try {
        const res = await fetch(`${apiBase}/review/${state.reviewId}/chat`, {
          method: "POST",
          headers: { "Content-Type": "application/json", ...getAuthHeaders() },
